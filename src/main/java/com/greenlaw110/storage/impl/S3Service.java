...
public class S3Service implements IStorageService {
     ...
     */
    public static final String S3_NO_GET = "storage.s3.noGet";
    ...
    public void configure(Map<String, String> conf) { 
        ...
        if (null == awsKeySecret || null == awsKeyId) {
            E.configException("AWS Key ID or AWS Key Secret not found in the configuration");
        }
        ...
        if (null == bucket) {
            E.configException("AWS bucket not found in the configuration");
        }
        ...
        if (conf.containsKey(S3_NO_GET)) {
            noGet = Boolean.parseBoolean(conf.get(S3_NO_GET));
        }
    }
...
//    }
    ...
    public ISObject get(String key) {
        if (noGet || S.empty(staticWebEndPoint)) {
            ...
            ISObject sobj = SObject.asSObject(key, s3obj.getObjectContent());
